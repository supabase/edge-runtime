...
pub trait LazyEszipV2:
  std::ops::Deref<Target = EszipV2> + AsyncEszipDataRead
{
}

impl<T> LazyEszipV2 for T where
  T: std::ops::Deref<Target = EszipV2> + AsyncEszipDataRead
{
}
  ...
  P: AsRef<Path>,
{
  ...
  let Some(eszip_static_files) = eszip.ensure_module(STATIC_FILES_ESZIP_KEY)
  else {
    return files;
  };
