...
impl IncrementalCache {
    pub fn new<TState: Serialize>(
        ...
        let result = IncrementalCacheInner::new(db_file_path, state, initial_file_paths);
...
impl SqlIncrementalCache {
    pub fn new(db_file_path: &Path, state_hash: u64) -> Result<Self, AnyError> {
        ...
        Self::from_connection(conn, state_hash, crate::util::version::cli())
    }
    ...
    fn get_source_hash_result(&self, path: &Path) -> Result<Option<u64>, AnyError> {
      ...
      LIMIT 1";
        ...
        let mut rows = stmt.query(params![path.to_string_lossy(), self.state_hash.to_string()])?;
...
fn create_tables(conn: &Connection, cli_version: String) -> Result<(), AnyError> {
...
mod test {
    ...
    pub fn sql_cache_general_use() {
        ...
        let cache = SqlIncrementalCache::from_connection(conn, 1, "1.0.0".to_string()).unwrap();
        ...
        let conn = cache.conn;
        let mut cache = SqlIncrementalCache::from_connection(conn, 1, "2.0.0".to_string()).unwrap();
        assert_eq!(cache.get_source_hash(&path), None);
        ...
        let conn = cache.conn;
        let cache = SqlIncrementalCache::from_connection(conn, 1, "2.0.0".to_string()).unwrap();
        assert_eq!(cache.get_source_hash(&path), Some(2));
    ...
    pub async fn incremental_cache_general_use() {
        let conn = Connection::open_in_memory().unwrap();
        let sql_cache = SqlIncrementalCache::from_connection(conn, 1, "1.0.0".to_string()).unwrap();
