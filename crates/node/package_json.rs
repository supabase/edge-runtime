...
impl PackageJson {
    ...
    pub fn load_skip_read_permission<Fs: NodeFs>(path: PathBuf) -> Result<PackageJson, AnyError> {
    ...
    pub fn load_from_string(path: PathBuf, source: String) -> Result<PackageJson, AnyError> {
        ...
        let dependencies = package_json.get("dependencies").and_then(|d| {
            if d.is_object() {
                let deps: HashMap<String, String> = serde_json::from_value(d.to_owned()).unwrap();
                Some(deps)
            } else {
                None
            }
