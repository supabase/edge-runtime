...
impl NodeAnalysisCache {
    ...
    pub fn get_cjs_analysis(
        ...
        self.with_inner(|inner| inner.get_cjs_analysis(specifier, expected_source_hash))
            .flatten()
    }

    pub fn set_cjs_analysis(&self, specifier: &str, source_hash: &str, cjs_analysis: &CjsAnalysis) {
        self.with_inner(|inner| inner.set_cjs_analysis(specifier, source_hash, cjs_analysis));
    }
    ...
    pub fn get_esm_analysis(
        ...
        self.with_inner(|inner| inner.get_esm_analysis(specifier, expected_source_hash))
            .flatten()
    }
    ...
    pub fn set_esm_analysis(
        ...
        self.with_inner(|inner| inner.set_esm_analysis(specifier, source_hash, top_level_decls));
    }
...
impl NodeAnalysisCacheInner {
    pub fn new(db_file_path: Option<&Path>, version: String) -> Result<Self, AnyError> {
    ...
    fn from_connection(conn: Connection, version: String) -> Result<Self, AnyError> {
    ...
    pub fn get_cjs_analysis(
      ...
      LIMIT 1";
        ...
        if let Some(row) = rows.next()? {
            ...
            let analysis_info: CjsAnalysisData = serde_json::from_str(&analysis_info)?;
            Ok(Some(CjsAnalysis {
...
mod test {
    ...
    pub fn node_analysis_cache_general_use() {
        ...
        let cache = NodeAnalysisCacheInner::from_connection(conn, "1.0.0".to_string()).unwrap();
        ...
        let actual_cjs_analysis = cache.get_cjs_analysis("file.js", "2").unwrap().unwrap();
        ...
        let actual_esm_analysis = cache.get_esm_analysis("file.js", "2").unwrap().unwrap();
        ...
        let conn = cache.conn;
        let cache = NodeAnalysisCacheInner::from_connection(conn, "1.0.0".to_string()).unwrap();
        let actual_analysis = cache.get_cjs_analysis("file.js", "2").unwrap().unwrap();
        ...
        let conn = cache.conn;
        let cache = NodeAnalysisCacheInner::from_connection(conn, "2.0.0".to_string()).unwrap();
        assert!(cache.get_cjs_analysis("file.js", "2").unwrap().is_none());
        assert!(cache.get_esm_analysis("file.js", "2").unwrap().is_none());
    }
}
