...
impl NodeResolver {
    ...
    pub fn resolve(
        ...
        let url =
            self.module_resolve(specifier, referrer, DEFAULT_CONDITIONS, mode, permissions)?;
        let url = match url {
            Some(url) => url,
            None => return Ok(None),
        };
        let url = match mode {
            ...
            NodeResolutionMode::Types => {
                ...
                let maybe_decl_url =
                    self.path_to_declaration_url(path, referrer, NodeModuleKind::Esm, permissions)?;
    ...
    fn module_resolve(
        ...
        let url = if should_be_treated_as_relative_or_absolute_path(specifier) {
            ...
            if mode.is_types() {
                ...
                self.path_to_declaration_url(file_path, referrer, NodeModuleKind::Esm, permissions)?
            } else {
    ...
    pub fn resolve_package_subpath_from_deno_module(
        ...
        let package_json = self.load_package_json(permissions, package_json_path.clone())?;
    ...
    pub fn resolve_binary_commands(&self, package_folder: &Path) -> Result<Vec<String>, AnyError> {
        ...
        let package_json =
            self.load_package_json(&AllowAllNodePermissions, package_json_path.clone())?;
    ...
    fn path_to_declaration_url(
        ...
        if self.fs.is_dir_sync(&path) {
            ...
            if let Ok(pkg_json) = self.load_package_json(permissions, package_json_path) {
    ...
    fn resolve_package_target(
        ...
        if let Some(target) = target.as_str() {
            ...
            if mode.is_types() && url.scheme() == "file" {
                let path = url.to_file_path().unwrap();
                return self.path_to_declaration_url(path, referrer, referrer_kind, permissions);
            } else {
                return Ok(Some(url));
            }
    ...
    pub(super) fn package_resolve(
        ...
        let Some(package_config) = self.get_closest_package_json(referrer, permissions)? else {
            return Ok(None);
        };
        // ResolveSelf
        ...
        // Package match.
        let package_json = self.load_package_json(permissions, package_json_path)?;
    ...
    fn resolve_package_subpath(
        ...
        if mode.is_types() {
            self.path_to_declaration_url(file_path, referrer, referrer_kind, permissions)
        } else {
    ...
    pub fn get_closest_package_json_from_path(
        ...
        let Some(package_json_path) = self.get_closest_package_json_path(file_path)? else {
            return Ok(None);
        };
    ...
    fn get_closest_package_json_path(&self, file_path: &Path) -> Result<Option<PathBuf>, AnyError> {
        let current_dir =
