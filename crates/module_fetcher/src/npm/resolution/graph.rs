...
impl GraphPath {
    ...
    pub fn get_path_to_ancestor_exclusive(&self, ancestor_node_id: NodeId) -> Vec<&Arc<GraphPath>> {
...
impl Graph {
    pub fn from_snapshot(snapshot: NpmResolutionSnapshot) -> Result<Self, AnyError> {
        fn get_or_create_graph_node(
            ...
            for (name, child_id) in &resolution.dependencies {
                let child_node_id =
                    get_or_create_graph_node(graph, child_id, packages, created_package_ids)?;
        ...
        let mut created_package_ids = HashMap::with_capacity(snapshot.packages.len());
    ...
    fn get_npm_pkg_id_from_resolved_id(
        ...
        if resolved_id.peer_dependencies.is_empty() {
            ...
            let mut npm_pkg_id = NpmPackageId {
                nv: (*resolved_id.nv).clone(),
                peer_dependencies: Vec::with_capacity(resolved_id.peer_dependencies.len()),
            };
            ...
            for peer_dep in &resolved_id.peer_dependencies {
                ...
                if let Some((child_id, child_resolved_id)) = maybe_node_and_resolved_id {
                    ...
                    if new_seen.insert(child_id) {
                        let child_peer = self
                            .get_npm_pkg_id_from_resolved_id(child_resolved_id, new_seen.clone());
    ...
    fn get_or_create_for_id(&mut self, resolved_id: &ResolvedId) -> (bool, NodeId) {
    ...
    fn set_child_of_parent_node(&mut self, parent_id: NodeId, specifier: &str, child_id: NodeId) {
    ...
    pub async fn into_snapshot(
        ...
        let packages_to_pkg_ids = self
            ...
            .collect::<HashMap<_, _>>();
        let mut copy_index_resolver = SnapshotPackageCopyIndexResolver::from_map_with_capacity(
    ...
    fn output_path(&self, path: &Arc<GraphPath>) {
        ...
        for path in path.ancestors() {
            match path {
                GraphPathNodeOrRoot::Node(node) => self.output_node(node.node_id(), false),
...
impl DepEntryCache {
    ...
    pub fn get(&self, id: &NpmPackageNv) -> Option<&Arc<Vec<NpmDependencyEntry>>> {
        self.0.get(id)
    }
}
...
pub struct GraphDependencyResolver<'a> {
    ...
    unresolved_optional_peers: HashMap<Arc<NpmPackageNv>, Vec<UnresolvedOptionalPeer>>,
...
impl<'a> GraphDependencyResolver<'a> {
    ...
    pub fn add_root_package(
        ...
        let version_req =
            VersionReq::parse_from_specifier(&format!("{}", package_nv.version)).unwrap();
        let (pkg_nv, node_id) =
            self.resolve_node_from_info(&package_nv.name, &version_req, package_info, None)?;
        self.graph.root_packages.insert(pkg_nv.clone(), node_id);
        self.pending_unresolved_nodes
    ...
    fn analyze_dependency(
        ...
        if child_id != parent_id {
            ...
            let new_path =
                parent_path.with_id(child_id, entry.bare_specifier.to_string(), child_nv);
            if let Some(ancestor) = maybe_ancestor {
                // this node is circular, so we link it to the ancestor
                self.add_linked_circular_descendant(&ancestor, new_path);
            } else {
                self.graph
                    .set_child_of_parent_node(parent_id, &entry.bare_specifier, child_id);
                ...
                match dep.kind {
                    NpmDependencyEntryKind::Dep => {
                        ...
                        let child_id = match node.children.get(&dep.bare_specifier) {
                            ...
                            None => self.analyze_dependency(dep, &package_info, &parent_path)?,
                        };
                        ...
                        if dep.kind == NpmDependencyEntryKind::OptionalPeer {
                            match maybe_new_id {
                                ...
                                None => {
                                    ...
                                    self.unresolved_optional_peers
    ...
    fn resolve_peer_dep(
        ...
        {
            let maybe_peer_dep =
                self.find_peer_dep_in_node(ancestor_path, peer_dep, peer_package_info)?;
    ...
    fn find_peer_dep_in_node(
            ...
            )?
        {
            ...
            find_matching_child(peer_dep, peer_package_info, children).map(|maybe_child_id| {
    ...
    fn add_peer_deps_to_path(
        ...
        for graph_path_node in path.iter().rev() {
            ...
            if created {
                let old_children = self.graph.borrow_node_mut(old_node_id).children.clone();
                // copy over the old children to this new one
                for (specifier, child_id) in &old_children {
                    self.graph
                        .set_child_of_parent_node(new_node_id, specifier, *child_id);
                }
            }

            graph_path_node.change_id(new_node_id);

            let circular_descendants = graph_path_node.linked_circular_descendants.lock().clone();
    ...
    fn set_new_peer_dep(
        ...
        self.graph
            .set_child_of_parent_node(bottom_node.node_id(), peer_dep_specifier, peer_dep_id);

        // queue next step
        let new_path =
            bottom_node.with_id(peer_dep_id, peer_dep_specifier.to_string(), peer_dep_nv);
...
mod test {
    ...
    async fn peer_deps_simple_top_tree() {
        ...
        let (packages, package_reqs) =
            run_resolver_and_get_output(api, vec!["npm:package-a@1.0", "npm:package-peer@1.0"])
                .await;
        assert_eq!(
            packages,
            vec![
    ...
    async fn resolve_with_optional_peer_dep_not_resolved() {
        ...
        api.add_dependency(("package-a", "1.0.0"), ("package-b", "^2"));
        api.add_dependency(("package-a", "1.0.0"), ("package-c", "^3"));
        api.add_optional_peer_dependency(("package-b", "2.0.0"), ("package-peer", "4"));
        api.add_optional_peer_dependency(("package-c", "3.0.0"), ("package-peer", "*"));
    ...
    async fn resolve_with_optional_peer_found() {
        ...
        let (packages, package_reqs) =
            run_resolver_and_get_output(api, vec!["npm:package-a@1", "npm:package-peer@4.0.0"])
                .await;
        assert_eq!(
            packages,
            vec![
    ...
    async fn resolve_optional_peer_first_not_resolved_second_resolved_scenario1() {
        ...
        api.add_optional_peer_dependency(("package-b", "1.0.0"), ("package-peer", "*"));

        let (packages, package_reqs) =
            run_resolver_and_get_output(api, vec!["npm:package-a@1", "npm:package-b@1"]).await;
        assert_eq!(
            packages,
            vec![
                TestNpmResolutionPackage {
    ...
    async fn resolve_optional_peer_first_not_resolved_second_resolved_scenario2() {
        ...
        api.add_optional_peer_dependency(("package-a", "1.0.0"), ("package-peer", "*"));
    ...
    async fn resolve_optional_dep_npm_req_top() {
        ...
        let (packages, package_reqs) =
            run_resolver_and_get_output(api, vec!["npm:package-a@1", "npm:package-peer@1"]).await;
        assert_eq!(
            packages,
            vec![
                TestNpmResolutionPackage {
    ...
    async fn resolve_nested_peer_deps_auto_resolved() {
        ...
        api.add_peer_dependency(("package-peer-a", "2.0.0"), ("package-peer-b", "3"));
    ...
    async fn resolve_with_peer_deps_multiple() {
        ...
        let (packages, package_reqs) =
            run_resolver_and_get_output(api, vec!["npm:package-0@1.1.1", "npm:package-e@3"]).await;
        assert_eq!(
      packages,
      vec![
        TestNpmResolutionPackage {
    ...
    async fn resolve_peer_deps_multiple_copies() {
        ...
        for _ in 0..3 {
            ...
            let (packages, package_reqs) =
                run_resolver_and_get_output(api, vec!["npm:package-a@1", "npm:package-b@2"]).await;
            assert_eq!(
                packages,
                vec![
                    TestNpmResolutionPackage {
    ...
    async fn resolve_dep_with_peer_deps_dep_then_peer() {
        ...
        let (packages, package_reqs) =
            run_resolver_and_get_output(api, vec!["npm:package-a@1.0", "npm:package-b@1.0"]).await;
        assert_eq!(
            packages,
            vec![
                TestNpmResolutionPackage {
                    pkg_id: "package-a@1.0.0_package-b@1.0.0__package-peer@1.0.0".to_string(),
                    copy_index: 0,
                    dependencies: BTreeMap::from([
                        (
                            "package-c".to_string(),
                ...
                TestNpmResolutionPackage {
                    pkg_id: "package-c@1.0.0_package-b@1.0.0__package-peer@1.0.0".to_string(),
                    copy_index: 0,
                    dependencies: BTreeMap::from([(
                        "package-b".to_string(),
                        "package-b@1.0.0_package-peer@1.0.0".to_string(),
    ...
    async fn nested_deps_same_peer_dep_ancestor() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                TestNpmResolutionPackage {
                    pkg_id: "package-b@1.0.0_package-0@1.0.0_package-a@1.0.0__package-0@1.0.0"
                        .to_string(),
                    copy_index: 0,
                    dependencies: BTreeMap::from([
                ...
                TestNpmResolutionPackage {
                    pkg_id: "package-c@1.0.0_package-0@1.0.0_package-a@1.0.0__package-0@1.0.0"
                        .to_string(),
                    copy_index: 0,
                    dependencies: BTreeMap::from([
                        ("package-0".to_string(), "package-0@1.0.0".to_string(),),
                ...
                TestNpmResolutionPackage {
                    pkg_id: "package-d@1.0.0_package-0@1.0.0_package-a@1.0.0__package-0@1.0.0"
                        .to_string(),
                    copy_index: 0,
                    dependencies: BTreeMap::from([
                        ("package-0".to_string(), "package-0@1.0.0".to_string(),),
    ...
    async fn peer_dep_resolved_then_resolved_deeper() {
        ...
        let (packages, package_reqs) =
            run_resolver_and_get_output(api, vec!["npm:package-0@1.0", "npm:package-peer@1.0"])
                .await;
        assert_eq!(
            packages,
            vec![
    ...
    async fn resolve_dep_with_peer_deps_circular_2() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                TestNpmResolutionPackage {
                    pkg_id: "package-c@1.0.0_package-b@1.0.0__package-a@1.0.0_package-a@1.0.0"
                        .to_string(),
                    copy_index: 0,
                    dependencies: BTreeMap::from([
                        (
                ...
                TestNpmResolutionPackage {
                    pkg_id: "package-d@1.0.0_package-b@1.0.0__package-a@1.0.0_package-a@1.0.0"
                        .to_string(),
                    copy_index: 0,
                    dependencies: BTreeMap::from([(
                        "package-c".to_string(),
                ...
                TestNpmResolutionPackage {
                    pkg_id: "package-e@1.0.0_package-a@1.0.0_package-b@1.0.0__package-a@1.0.0"
                        .to_string(),
                    copy_index: 0,
                    dependencies: BTreeMap::from([(
                        "package-f".to_string(),
                ...
                TestNpmResolutionPackage {
                    pkg_id: "package-f@1.0.0_package-a@1.0.0_package-b@1.0.0__package-a@1.0.0"
                        .to_string(),
                    copy_index: 0,
                    dependencies: BTreeMap::from([
    ...
    async fn resolve_dep_with_peer_deps_circular_3() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                TestNpmResolutionPackage {
                    pkg_id: "package-d@1.0.0_package-c@1.0.0__package-a@1.0.0_package-a@1.0.0"
                        .to_string(),
                    copy_index: 0,
                    dependencies: BTreeMap::from([
                        (
