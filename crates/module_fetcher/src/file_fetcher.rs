...
mod tests {
    ...
    async fn test_fetch_remote(specifier: &ModuleSpecifier) -> (File, HashMap<String, String>) {
        ...
        let result: Result<File, AnyError> = file_fetcher
            .fetch_remote(specifier, Permissions::allow_all(), 1, None)
            .await;
        assert!(result.is_ok());
    ...
    async fn test_fetch_local_encoded(charset: &str, expected: String) {
        ...
        let specifier = ModuleSpecifier::from_file_path(p.to_str().unwrap()).unwrap();
    ...
    async fn test_insert_cached() {
        ...
        let specifier =
            ModuleSpecifier::from_file_path(local.as_os_str().to_str().unwrap()).unwrap();
        let file = File {
            local,
            maybe_types: None,
    ...
    async fn test_fetch_redirected() {
        ...
        let (_, headers, _) = file_fetcher.http_cache.get(&redirected_specifier).unwrap();
    ...
    async fn test_fetch_uses_cache_with_redirects() {
        ...
        let metadata_filename = CachedUrlMetadata::filename(&redirected_cache_filename);
        let metadata_file = fs::File::open(metadata_filename).unwrap();
        let metadata_file_metadata = metadata_file.metadata().unwrap();
        let metadata_file_modified_01 = metadata_file_metadata.modified().unwrap();
    ...
    async fn test_fetcher_limits_redirects() {
        ...
        let result = file_fetcher
            .fetch_remote(&specifier, Permissions::allow_all(), 2, None)
            .await;
        assert!(result.is_ok());
    ...
    async fn test_fetch_remote_javascript_with_types() {
        let specifier =
            ModuleSpecifier::parse("http://127.0.0.1:4545/xTypeScriptTypes.js").unwrap();
    ...
    async fn test_fetch_remote_jsx_with_types() {
        let specifier =
            ModuleSpecifier::parse("http://127.0.0.1:4545/xTypeScriptTypes.jsx").unwrap();
        let (file, _) = test_fetch_remote(&specifier).await;
    ...
    async fn test_fetch_remote_typescript_with_types() {
        let specifier =
            ModuleSpecifier::parse("http://127.0.0.1:4545/xTypeScriptTypes.ts").unwrap();
        let (file, _) = test_fetch_remote(&specifier).await;
        assert_eq!(file.maybe_types, None);
    }
    ...
    fn create_test_client() -> HttpClient {
        HttpClient::from_client(
            create_http_client("test_client", None, vec![], None, None, None).unwrap(),
        )
    }
