...
#[op]
pub fn op_create_brotli_compress(state: &mut OpState, params: Vec<(u8, i32)>) -> u32 {
    let inst =
...
fn brotli_decompress(buffer: &[u8]) -> Result<ToJsBuffer, AnyError> {
    ...
    loop {
        ...
        // SAFETY: TODO(littledivy)
        match unsafe {
            CBrotliDecoderDecompress(in_size, in_buffer, &mut out_size as *mut usize, out_buffer)
        } {
...
#[op]
pub async fn op_brotli_decompress_async(buffer: JsBuffer) -> Result<ToJsBuffer, AnyError> {
