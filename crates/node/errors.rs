...
pub fn err_invalid_module_specifier(
    ...
    let mut msg = format!("[ERR_INVALID_MODULE_SPECIFIER] Invalid module \"{request}\" {reason}");
...
pub fn err_invalid_package_config(
    ...
    let mut msg = format!("[ERR_INVALID_PACKAGE_CONFIG] Invalid package config {path}");
...
pub fn err_package_path_not_exported(
    ...
    if subpath == "." {
        msg = format!("{msg} No \"exports\" main defined{types_msg} in '{pkg_path}package.json'");
    } else {
