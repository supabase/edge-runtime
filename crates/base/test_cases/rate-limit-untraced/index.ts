// Worker C: calls itself repeatedly to exhaust the global (untraced) budget.
// Supports two outbound HTTP modes via x-http-mode: "fetch" (default) or "node".
  ...
  try {
    ...
    if (mode === "node") {
      ({ status, body } = await requestViaNode(
        `${serverUrl}/rate-limit-untraced`,
        forwardHeaders,
      ));
    } else {
      const resp = await fetch(`${serverUrl}/rate-limit-untraced`, {
