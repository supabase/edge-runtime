...
impl GlobalNpmPackageResolver {
    pub fn new(cache: NpmCache, registry_url: Url, resolution: NpmResolution) -> Self {
        Self {
            cache,
            resolution,
            registry_url,
...
async fn cache_packages_in_resolver(resolver: &GlobalNpmPackageResolver) -> Result<(), AnyError> {
    ...
    for copy in package_partitions.copy_packages {
        resolver
            .cache
            .ensure_copy_package(&copy.get_package_cache_folder_id(), &resolver.registry_url)?;
    }
