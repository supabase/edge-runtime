...
type CreateFutureFn<TResult> = Box<dyn Fn() -> LocalBoxFuture<'static, TResult> + Send + Sync>;
...
impl<TResult: Send + Clone + 'static> std::fmt::Debug for MultiRuntimeAsyncValueCreator<TResult> {
...
impl<TResult: Send + Clone + 'static> MultiRuntimeAsyncValueCreator<TResult> {
    ...
    fn create_shared_future(&self) -> Shared<BoxFuture<'static, JoinResult<TResult>>> {
...
mod test {
    ...
    async fn single_runtime() {
        let value_creator =
            MultiRuntimeAsyncValueCreator::new(Box::new(|| async { 1 }.boxed_local()));
    ...
    #[test]
    fn multi_runtimes() {
        let value_creator = Arc::new(MultiRuntimeAsyncValueCreator::new(Box::new(|| {
            async {
