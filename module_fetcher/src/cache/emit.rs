...
impl EmitCache {
    pub fn new(disk_cache: DiskCache) -> Self {
        Self {
            disk_cache,
            cli_version: crate::util::version::cli(),
        }
    }
...
mod test {
    ...
    pub fn emit_cache_general_use() {
        ...
        let specifier1 = ModuleSpecifier::from_file_path(temp_dir.path().join("file1.ts")).unwrap();
        let specifier2 = ModuleSpecifier::from_file_path(temp_dir.path().join("file2.ts")).unwrap();
