...
impl FileFetcher {
    ...
    fn fetch_data_url(&self, specifier: &ModuleSpecifier) -> Result<File, AnyError> {
        ...
        let (media_type, _) = map_content_type(specifier, Some(content_type.clone()));
    ...
    async fn fetch_blob_url(&self, specifier: &ModuleSpecifier) -> Result<File, AnyError> {
        ...
        let (media_type, maybe_charset) = map_content_type(specifier, Some(content_type.clone()));
        let source = get_source_from_bytes(bytes, maybe_charset)?;
...
async fn fetch_once(
    ...
    let body = http_util::get_response_body(response).await?;
