    ...
    P: NodePermissions + 'static,
{
    ...
    let pkg = node_resolver
        .load_package_json(permissions, PathBuf::from(&pkg_path).join("package.json"))?;
    ...
    P: NodePermissions + 'static,
{
    ...
    let permissions = state.borrow::<P>();
    node_resolver
        .get_closest_package_json(&Url::from_file_path(filename).unwrap(), permissions)
    ...
    P: NodePermissions + 'static,
{
    ...
    let permissions = state.borrow::<P>();
    let Some(pkg) =
        node_resolver.get_closest_package_json_from_path(&referrer_path, permissions)?
    else {
        return Ok(None);
    };
