...
impl NodeResolver {
    ...
    pub fn resolve(
        ...
        let url = match mode {
            ...
            NodeResolutionMode::Types => {
                ...
                let path = match self.path_to_declaration_path(path, NodeModuleKind::Esm) {
                    Some(path) => path,
                    None => return Ok(None),
                };
    ...
    pub fn resolve_npm_reference(
        ...
        let maybe_resolved_path = self
            ...
            .with_context(|| format!("Failed resolving package config for '{reference}'"))?;
    ...
    pub fn resolve_binary_commands(&self, pkg_nv: &NpmPackageNv) -> Result<Vec<String>, AnyError> {
        ...
        let package_json = self.load_package_json(&AllowAllNodePermissions, package_json_path)?;
    ...
    fn package_config_resolve(
        ...
        let package_config = self.load_package_json(permissions, package_json_path.clone())?;
    ...
    pub(super) fn package_imports_resolve(
        ...
        if let Some(package_config) = self.get_package_scope_config(referrer, permissions)? {
    ...
    pub(super) fn package_resolve(
      ...
      self.get_package_scope_config(referrer, permissions)? else {
        ...
        if mode.is_types() {
            let maybe_declaration_path = self.path_to_declaration_path(file_path, referrer_kind);
            ...
            match package_json.types.as_ref() {
                ...
                None => {
                    ...
                    if let Some(main) = package_json.main(referrer_kind) {
                        ...
                        if let Some(path) = self.path_to_declaration_path(main, referrer_kind) {
                            return Ok(Some(path));
                        }
                    }
                    None
