...
import {
  initWasm,
  Resvg,
} from "https://esm.sh/@resvg/resvg-wasm@2.4.1";
...
const resvg_wasm_url = "https://esm.sh/@resvg/resvg-wasm@2.4.1/index_bg.wasm";
...
const initializedResvg = initWasm(resvg_wasm_url);
...
export async function renderImage(element: ReactElement, options: ImageResponseOptions = {}) {
  const extendedOptions = Object.assign(
      {
        width: 1200,
        height: 630,
  ...
  return new Response(new Blob([res.asPng()], { type: 'image/png' }), { headers: {
      "content-type": "image/png",
      "cache-control": isDev
        ? "no-cache, no-store"
        : "public, max-age=31536000, no-transform, immutable",
