        ...
        if label.is_ascii() {
            ...
            idna::punycode::encode_str(label)
                ...
                .ok_or_else(|| {
                    ...
                    // only error possible per the docs
                })
        }
    })?;
...
fn to_unicode(input: &str) -> Result<String, Error> {
    map_domain(input, |s| {
        if let Some(puny) = s.strip_prefix(PUNY_PREFIX) {
            ...
            Ok(idna::punycode::decode_to_string(&puny.to_lowercase())
...
#[op2]
#[string]
pub fn op_node_idna_punycode_to_ascii(#[string] domain: String) -> Result<String, Error> {
    to_ascii(&domain)
}
...
#[op2]
#[string]
pub fn op_node_idna_punycode_to_unicode(#[string] domain: String) -> Result<String, Error> {
    to_unicode(&domain)
}
...
#[op2]
#[string]
pub fn op_node_idna_domain_to_ascii(#[string] domain: String) -> Result<String, Error> {
...
#[op2]
#[string]
pub fn op_node_idna_punycode_decode(#[string] domain: String) -> Result<String, Error> {
    if domain.is_empty() {
        return Ok(domain);
    }
