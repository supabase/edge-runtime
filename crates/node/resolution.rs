...
pub fn path_to_declaration_path<Fs: NodeFs>(
    ...
    fn probe_extensions<Fs: NodeFs>(path: &Path, referrer_kind: NodeModuleKind) -> Option<PathBuf> {
    ...
    if path.is_dir() {
        if let Some(path) = probe_extensions::<Fs>(&path.join("index"), referrer_kind) {
            return Some(path);
        }
    }
    None
...
pub fn package_imports_resolve<Fs: NodeFs>(
    ...
    let package_config = get_package_scope_config::<Fs>(referrer, npm_resolver, permissions)?;
...
fn resolve_package_target_string<Fs: NodeFs>(
    ...
    if !target.starts_with("./") {
        if internal && !target.starts_with("../") && !target.starts_with('/') {
            ...
            if !is_url {
                ...
                let package_json_url = ModuleSpecifier::from_file_path(package_json_path).unwrap();
    ...
    if pattern {
        ...
        let replaced = pattern_re.replace(&resolved_path_str, |_caps: &regex::Captures| {
            subpath.clone()
        });
...
pub fn package_resolve<Fs: NodeFs>(
    ...
    // Package match.
    let package_json = PackageJson::load::<Fs>(npm_resolver, permissions, package_json_path)?;
    ...
    if mode.is_types() {
        let maybe_declaration_path = path_to_declaration_path::<Fs>(file_path, referrer_kind);
...
pub fn get_package_scope_config<Fs: NodeFs>(
    ...
    let root_folder =
        npm_resolver.resolve_package_folder_from_path(&referrer.to_file_path().unwrap())?;
...
pub fn get_closest_package_json<Fs: NodeFs>(
    ...
    let package_json_path = get_closest_package_json_path::<Fs>(url, npm_resolver)?;
    PackageJson::load::<Fs>(npm_resolver, permissions, package_json_path)
}
...
fn get_closest_package_json_path<Fs: NodeFs>(
    ...
    let root_pkg_folder =
        npm_resolver.resolve_package_folder_from_path(&url.to_file_path().unwrap())?;
        ...
        match package_json.types.as_ref() {
            ...
            None => {
                ...
                if let Some(main) = package_json.main(referrer_kind) {
                    ...
                    if let Some(path) = path_to_declaration_path::<Fs>(main, referrer_kind) {
                        return Ok(Some(path));
                    }
                }
                None
...
mod tests {
    ...
    fn test_parse_package_name() {
        ...
        assert_eq!(
            parse_package_name("@astrojs/prism/dist/highlighter", &dummy_referrer).unwrap(),
            (
