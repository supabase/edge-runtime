...
thread_local! {
  ...
  pub static DESCRIPTOR_MAP_FN: v8::NamedPropertyDescriptorCallback<'static> = descriptor.map_fn_to();
}
...
pub fn global_object_middleware<'s>(
    ...
    let object_key = v8::String::new_external_onebyte_static(scope, b"Object").unwrap();
    let object = global
        .get(scope, object_key.into())
        .unwrap()
        .to_object(scope)
        .unwrap();
    let prototype_key = v8::String::new_external_onebyte_static(scope, b"prototype").unwrap();
    ...
    let reflect_key = v8::String::new_external_onebyte_static(scope, b"Reflect").unwrap();
    let reflect = global
    ...
    let bootstrap_key = v8::String::new_external_onebyte_static(scope, b"__bootstrap").unwrap();
    ...
    let deno_globals_key =
        v8::String::new_external_onebyte_static(scope, b"ext_node_denoGlobals").unwrap();
    let deno_globals = match bootstrap.get(scope, deno_globals_key.into()) {
        Some(value) if value.is_object() => value,
        Some(value) if value.is_undefined() => {
            let null = v8::null(scope);
            let obj =
                v8::Object::with_prototype_and_properties(scope, null.into(), &[], &[]).into();
    ...
    let node_globals_key =
        v8::String::new_external_onebyte_static(scope, b"ext_node_nodeGlobals").unwrap();
...
pub fn query<'a, 'b,'s>(
    ...
    mut rv: v8::ReturnValue<'b, v8::Integer>,
) -> v8::Intercepted {
    if !is_managed_key(scope, key) {
        return v8::Intercepted::No;
    };
...
pub fn deleter<'a, 'b, 's>(
    scope: &'a mut v8::HandleScope<'s>,
    key: v8::Local<'s, v8::Name>,
    args: v8::PropertyCallbackArguments<'s>,
    mut rv: v8::ReturnValue<'b, v8::Boolean>,
) -> v8::Intercepted {
    if !is_managed_key(scope, key) {
        return v8::Intercepted::No;
    };
...
pub fn enumerator<'a, 'b, 's>(
    scope: &'a mut v8::HandleScope<'s>,
    _args: v8::PropertyCallbackArguments<'s>,
    mut rv: v8::ReturnValue<'b, v8::Array>,
) {
    let mode = current_mode(scope);
...
pub fn definer<'a, 'b, 'c, 's>(
    scope: &'a mut v8::HandleScope<'s>,
    key: v8::Local<'s, v8::Name>,
    descriptor: &'b v8::PropertyDescriptor,
    args: v8::PropertyCallbackArguments<'s>,
    _rv: v8::ReturnValue<'c, ()>,
) -> v8::Intercepted {
    if !is_managed_key(scope, key) {
        return v8::Intercepted::No;
    };
