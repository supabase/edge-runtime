...
#[op]
fn op_require_stat<Env>(state: &mut OpState, path: String) -> Result<i32, AnyError>
where
    Env: NodeEnv + 'static,
{
...
#[op]
fn op_require_real_path<Env>(state: &mut OpState, request: String) -> Result<String, AnyError>
where
    Env: NodeEnv + 'static,
{
...
#[op]
fn op_require_read_file<Env>(state: &mut OpState, file_path: String) -> Result<String, AnyError>
where
    Env: NodeEnv + 'static,
{
    ...
    Env: NodeEnv + 'static,
{
    ...
    let pkg_path =
        if resolver.in_npm_package(&PathBuf::from(&modules_path)) && !uses_local_node_modules_dir {
            modules_path
        } else {
...
where
    Env: NodeEnv + 'static,
{
    ensure_read_permission::<Env::P>(state, PathBuf::from(&filename).parent().unwrap())?;
    let resolver = state.borrow::<Rc<dyn RequireNpmResolver>>().clone();
    let permissions = state.borrow_mut::<Env::P>();
    ...
    Env: NodeEnv + 'static,
{
    ...
    let permissions = state.borrow_mut::<Env::P>();
    let pkg =
        PackageJson::load::<Env::Fs>(&*resolver, permissions, parent_path.join("package.json"))?;
