...
fn ensure_read_permission<P>(state: &mut OpState, file_path: &Path) -> Result<(), AnyError>
where
...
pub fn op_require_resolve_deno_dir(
    ...
    resolver
        .resolve_package_folder_from_package(
            &request,
            &ModuleSpecifier::from_file_path(&parent_filename)
                .unwrap_or_else(|_| panic!("Url::from_file_path: [{:?}]", parent_filename)),
        )
        .ok()
        .map(|p| p.to_string_lossy().to_string())
}
...
#[op2(fast)]
pub fn op_require_stat<P>(state: &mut OpState, #[string] path: String) -> Result<i32, AnyError>
where
    P: NodePermissions + 'static,
{
...
#[op2]
#[string]
pub fn op_require_path_dirname(#[string] request: String) -> Result<String, AnyError> {
...
#[op2]
#[string]
pub fn op_require_path_basename(#[string] request: String) -> Result<String, AnyError> {
    let p = PathBuf::from(request);
    ...
    P: NodePermissions + 'static,
{
    ...
    let pkg = node_resolver
        .get_closest_package_json(&Url::from_file_path(parent_path.unwrap()).unwrap())
        .ok()
        .flatten();
    if pkg.is_none() {
        return Ok(None);
    ...
    P: NodePermissions + 'static,
{
    ...
    let Some(pkg) =
        node_resolver.load_package_json(&PathBuf::from(&pkg_path).join("package.json"))?
    else {
        return Ok(None);
    };
...
where
    P: NodePermissions + 'static,
{
    ensure_read_permission::<P>(state, PathBuf::from(&filename).parent().unwrap())?;
    ...
    P: NodePermissions + 'static,
{
    ...
    let node_resolver = state.borrow::<NodeResolverRc>();
    let Some(pkg) = node_resolver.get_closest_package_json_from_path(&referrer_path)? else {
        return Ok(None);
    };

    if pkg.imports.is_some() {
        let referrer_url = deno_core::url::Url::from_file_path(&referrer_filename).unwrap();
