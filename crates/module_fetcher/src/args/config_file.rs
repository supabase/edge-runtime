...
mod tests {
    ...
    fn test_parse_config() {
      ...
      "tasks": {
        ...
        let (options_value, ignored) = config_file.to_compiler_options().expect("error parsing");
    ...
    fn test_parse_config_with_empty_file() {
        ...
        let config_file = ConfigFile::new(config_text, &config_specifier).unwrap();
        let (options_value, _) = config_file.to_compiler_options().expect("error parsing");
        assert!(options_value.is_object());
    }
    ...
    fn discover_from_success() {
        ...
        let expected_exclude =
            ModuleSpecifier::from_file_path(testdata.join("fmt/with_config/subdir/b.ts"))
                .unwrap()
                .to_file_path()
                .unwrap();
