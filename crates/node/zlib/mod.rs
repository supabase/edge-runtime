...
impl ZlibInner {
    ...
    fn do_write(&mut self, flush: Flush) -> Result<(), AnyError> {
        ...
        match self.mode {
            ...
            Mode::Unzip if self.strm.avail_in > 0 => 'blck: {
                ...
                // `start_write`.
                let strm = unsafe {
                    std::slice::from_raw_parts(self.strm.next_in, self.strm.avail_in as usize)
                };
                ...
                if self.gzib_id_bytes_read == 1 {
                    ...
                    return Err(type_error("invalid number of gzip magic number bytes read"));
                }
            }
            _ => {}
        }
    ...
    fn init_stream(&mut self) -> Result<(), AnyError> {
        ...
        self.err = match self.mode {
            Mode::Deflate | Mode::Gzip | Mode::DeflateRaw => {
                self.strm
                    .deflate_init(self.level, self.window_bits, self.mem_level, self.strategy)
            }
...
pub fn op_zlib_write_async(
    ...
    out_off: u32,
    out_len: u32,
) -> Result<impl Future<Output = Result<(i32, u32, u32), AnyError>> + 'static, AnyError> {
