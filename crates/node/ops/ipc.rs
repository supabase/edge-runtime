...
mod impl_ {
    ...
    #[op2]
    #[smi]
    pub fn op_node_child_ipc_pipe(state: &mut OpState) -> Result<Option<ResourceId>, AnyError> {
    ...
    fn pipe(stream: RawFd) -> Result<(OwnedReadHalf, OwnedWriteHalf), io::Error> {
        ...
        // implementation.
        let unix_stream =
            UnixStream::from_std(unsafe { std::os::unix::net::UnixStream::from_raw_fd(stream) })?;
    ...
    impl IpcJsonStreamResource {
        ...
        async fn write_msg(self: Rc<Self>, msg: serde_json::Value) -> Result<(), AnyError> {
    ...
    impl IpcJsonStream {
        ...
        async fn read_msg(&mut self) -> Result<serde_json::Value, AnyError> {
            let mut json = None;
            let nread = read_msg_inner(&mut self.pipe, &mut self.buffer, &mut json).await?;
            if nread == 0 {
                // EOF.
    ...
    mod neon {
        ...
        pub unsafe fn neon_memchr(str: &[u8], c: u8, length: usize) -> Option<usize> {
    ...
    mod tests {
        ...
        pub async fn pair() -> (
            ...
            let name = format!(r"\\.\pipe\deno-named-pipe-test-{}", rand::random::<u32>());
