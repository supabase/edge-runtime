...
impl LocalNpmPackageResolver {
    ...
    fn get_package_id_folder(&self, id: &NpmPackageId) -> Result<PathBuf, AnyError> {
...
impl NpmPackageFsResolver for LocalNpmPackageResolver {
    ...
    fn resolve_package_folder_from_package(
        ...
        loop {
            ...
            if mode.is_types() && !name.starts_with("@types/") {
                let sub_dir = join_package_name(current_folder, &types_package_name(name));
                if sub_dir.is_dir() {
                    return Ok(sub_dir);
                }
            }
    ...
    fn ensure_read_permission(
        ...
        ensure_registry_read_permission(permissions, &self.root_node_modules_path, path)
    }
}
...
async fn sync_resolution_with_fs(
    ...
    fs::create_dir_all(&deno_local_registry_dir)
    ...
    for package in &package_partitions.packages {
            ...
            || !initialized_file.exists()
        {
            ...
            let handle = tokio::task::spawn(async move {
                ...
                let package_path = join_package_name(&sub_node_modules, &package.pkg_id.nv.name);
                fs::create_dir_all(&package_path)
                    .with_context(|| format!("Creating '{}'", folder_path.display()))?;
                let cache_folder =
                    cache.package_folder_for_name_and_version(&package.pkg_id.nv, &registry_url);
