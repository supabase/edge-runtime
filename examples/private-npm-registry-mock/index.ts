...
const PREFIX = "private-npm-registry-mock";
...
export default {
    async fetch(req: Request) {
        ...
        if (ucEnc(pathname) === `/${PREFIX}/${softEncode(PKG_NAME)}` || ucEnc(pathname) === `/${PREFIX}/${PKG_NAME}`) {
            return Response.json(generatePackageResponse("localhost", moduleName, sum));
        }
...
function generatePackageResponse(hostname: string, moduleName: string, sum: string) {
