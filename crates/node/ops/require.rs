...
#[op]
fn op_require_stat<P>(state: &mut OpState, path: String) -> Result<i32, AnyError>
where
    P: NodePermissions + 'static,
{
...
#[op]
fn op_require_real_path<P>(state: &mut OpState, request: String) -> Result<String, AnyError>
where
    P: NodePermissions + 'static,
{
...
#[op]
fn op_require_read_file<P>(state: &mut OpState, file_path: String) -> Result<String, AnyError>
where
    P: NodePermissions + 'static,
{
    ...
    P: NodePermissions + 'static,
{
    ...
    let node_resolver = state.borrow::<Rc<NodeResolver>>();
    let permissions = state.borrow::<P>();
    node_resolver.get_closest_package_json(&Url::from_file_path(filename).unwrap(), permissions)
}
    ...
    P: NodePermissions + 'static,
{
    ...
    let node_resolver = state.borrow::<Rc<NodeResolver>>();
    let permissions = state.borrow::<P>();
    let pkg = node_resolver.load_package_json(permissions, parent_path.join("package.json"))?;

    if pkg.imports.is_some() {
        let referrer = deno_core::url::Url::from_file_path(&parent_filename).unwrap();
        node_resolver
