...
impl TypeCheckCache {
    ...
    fn try_new(db_file_path: &Path) -> Result<Self, AnyError> {
        ...
        Self::from_connection(conn, crate::util::version::cli())
    }

    fn from_connection(conn: Connection, cli_version: String) -> Result<Self, AnyError> {
...
mod test {
    ...
    pub fn check_cache_general_use() {
        ...
        let cache = TypeCheckCache::from_connection(conn, "1.0.0".to_string()).unwrap();
        ...
        let cache = TypeCheckCache::from_connection(conn, "2.0.0".to_string()).unwrap();
        assert!(!cache.has_check_hash(1));
        cache.add_check_hash(1);
        assert!(cache.has_check_hash(1));
        assert_eq!(cache.get_tsbuildinfo(&specifier1), None);
