...
mod map_to_vec {
    ...
    pub fn deserialize<'de, D, K: Deserialize<'de> + Eq + std::hash::Hash, V: Deserialize<'de>>(
        deserializer: D,
...
impl NpmResolutionSnapshot {
    ...
    pub fn resolve_package_from_package(
        ...
        let referrer_package = self
            ...
            .ok_or_else(|| anyhow!("could not find referrer npm package '{}'", referrer))?;
    ...
    pub async fn from_lockfile(
...

        {
            ...
            package_reqs = HashMap::with_capacity(lockfile.content.npm.specifiers.len());
            root_packages = HashMap::with_capacity(lockfile.content.npm.specifiers.len());
            ...
            copy_index_resolver = SnapshotPackageCopyIndexResolver::with_capacity(packages_len);
            ...
            for id in &verify_ids {
                if !packages.contains_key(id) {
                    bail!("the lockfile is corrupt. You can recreate it with --lock-write");
                }
            }
        }

        api.cache_in_parallel(packages_by_name.keys().cloned().collect())
            .await?;

        // ensure the dist is set for each package
        for package in packages.values_mut() {
            // this will read from the memory cache now
            let version_info = match api.package_version_info(&package.pkg_id.nv).await? {
...
impl SnapshotPackageCopyIndexResolver {
    ...
    pub fn from_map_with_capacity(
        ...
        let mut package_name_version_to_copy_count = HashMap::with_capacity(capacity); // close enough
...
mod tests {
    ...
    fn test_copy_index_resolver() {
        let mut copy_index_resolver = SnapshotPackageCopyIndexResolver::with_capacity(10);
        assert_eq!(
            copy_index_resolver.resolve(&NpmPackageId::from_serialized("package@1.0.0").unwrap()),
            0
        );
        assert_eq!(
            copy_index_resolver.resolve(&NpmPackageId::from_serialized("package@1.0.0").unwrap()),
        ...
        assert_eq!(
            copy_index_resolver
                .resolve(&NpmPackageId::from_serialized("package@1.0.0_package-b@1.0.0").unwrap()),
            1
        );
        assert_eq!(
            copy_index_resolver.resolve(
                &NpmPackageId::from_serialized("package@1.0.0_package-b@1.0.0__package-c@2.0.0")
                    .unwrap()
            ),
            2
        );
        ...
        assert_eq!(
            copy_index_resolver.resolve(&NpmPackageId::from_serialized("package-b@1.0.0").unwrap()),
            0
        );
    }
}
